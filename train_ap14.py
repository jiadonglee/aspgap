...
if __name__ == "__main__":
    ...
    aspcap  = ASPCAP(data_dir+tr_file, device=device)
    # aspcap_val = ASPCAP(data_dir+val_file, device=device)
    train_size = int(0.75*len(aspcap))
    val_size = len(aspcap) - train_size
    train_dataset, val_dataset = torch.utils.data.random_split(aspcap, [train_size, val_size], 
    ...
    torch.save(model.state_dict(), data_dir+"model/ap_prlx_trsfm_221019.pt")
