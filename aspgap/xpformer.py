...
class XPAPformerConv(nn.Module):
    def __init__(self, input1_size, input2_size, n_outputs, input_proj=True, output_proj=False, 
                 hidden_size=16, channels=8, num_heads=2, num_layers=2,
                 dropout=0.1, device=torch.device('cpu')):
        super(XPAPformerConv, self).__init__()
        self.input1_size = input1_size
        self.input2_size = input2_size
        self.num_dim = channels
        self.n_outputs = n_outputs
        ...
        self.fc1 = nn.Linear(4*channels*(input1_size+input2_size), hidden_size,  bias=False)
        self.fc2 = nn.Linear(hidden_size, n_outputs, bias=False)
...
class XPAPformerConv2(nn.Module):
    def __init__(self, input1_size, input2_size, n_outputs, input_proj=True, output_proj=False, 
                 hidden_size=16, channels=8, num_heads=2, num_layers=2,
                 dropout=0.1, device=torch.device('cpu')):
        super(XPAPformerConv2, self).__init__()
        self.input1_size = input1_size
        self.input2_size = input2_size
        self.num_dim = channels
        self.n_outputs = n_outputs
        ...
        self.input_project = nn.Linear(1, self.num_dim)
        # self.out_project = nn.Linear(1, self.num_dim)
        self.conv1 = nn.Conv1d(channels, channels, kernel_size=5, stride=1, padding=1)
        self.conv2 = nn.Conv1d(channels, 4*channels, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv1d(4*channels, channels, kernel_size=3, stride=1, padding=2)
        self.fc1 = nn.Linear(channels*(input1_size+input2_size), hidden_size,  bias=False)
        self.fc2 = nn.Linear(hidden_size, n_outputs, bias=False)
