...
if __name__ == "__main__":
    ...
    model = xp2label(n_encoder_inputs=INPUT_LEN, 
    n_outputs=1, channels=n_dim, n_heads=n_head, n_layers=n_layer).to(device)
    ...
    def train_epoch(tr_loader, epoch):
        ...
        for batch, data in enumerate(tr_loader):
            output = model(data['x'])
            loss = cost(output.view(-1,1), data['y'][:,idx_1l].view(-1,1)+20)
