...
class XPAP4l():
    ...
    def __init__(self, data_path, total_num=6000, part_train=False, device=torch.device('cpu')) -> None:
        ...
        self.source_id  = self.data['source_id'].astype(np.int64)
        self.total_num  = total_num
        self.part_train = part_train
        self.device = device
    ...
    def __getitem__(self, idx: int):
        ...
        return {'xp':xp, 'ap':ap, 'y':y, 'e_y':e_y, 'id':self.source_id[idx]}
...
class XPAP4l_infer():
    ...
    def __getitem__(self, idx: int):
        ...
        return {'xp':xp, 'snr_rp':self.snr_rp[idx], 'id':self.source_id[idx]}
...
class XPAP4l_v2():
    ...
    def __init__(self, data_path, total_num=6000, part_train=False, device=torch.device('cpu')) -> None:
        ...
        self.labels = self.data['y']
        self.e_labels   = self.data['e_y']
        self.tmass_id  = self.data['tmass_id']
        self.source_id = self.data['source_id'].astype(np.int64)
        self.total_num  = total_num
        self.part_train = part_train
        self.device = device
    ...
    def __getitem__(self, idx: int):
        ...
        tmass_id  = self.tmass_id[idx]
        
        return {'xp':xp, 'ap':ap, 
                'y':y, 'e_y':e_y, 
                'source_id':source_id, 'tmass_id':tmass_id}