...
def draw_hist2d(ax, true, pred, xrange=[-2, 0.5], C=None, bins=100, cmap='cmr.dusk_r', text_stats=True, **kwargs):
    
    xx = np.linspace(xrange[0], xrange[1])
    res = pred-true
    ax.plot(xx, xx, ls='--', lw=2, c='k', zorder=5)
    ...
    ax2.hist2d(true, res, cmap=cmap, bins=bins, zorder=5, norm=colors.LogNorm())
    ax2.axhline(y=0, c='k', zorder=6, lw=3, ls="--")
    ax2.axhline(y=np.percentile(res, 14), c='k', zorder=6, lw=1, ls=':')
    ax2.axhline(y=np.percentile(res, 86), c='k', zorder=6, lw=1, ls=':')
    # ax2.set_xlabel(r"APOGEE");
    ax2.set_ylim(xrange);
    ax2.set_xlim(xrange);
    ax.set_xticks([]);
...
def draw_pars(ax1, ax2, teff, logg, moh, aom, gridsize=(50,50), C1=None, C2=None, vmin=None, vmax=None, cmap='cmr.prinsenvlag_r'):
    ...
    if C1 is None and C2 is None:
        ...
        clb2 = plt.colorbar(img2, ax=ax2, extend='both')
